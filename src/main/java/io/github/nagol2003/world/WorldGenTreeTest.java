...
public class WorldGenTreeTest extends WorldGenHugeTrees {
	public WorldGenTreeTest(boolean notify, int baseHeightIn, int extraRandomHeightIn, IBlockState woodMetadataIn,
	...
	public WorldGenTreeTest(boolean notify, int baseHeightIn, int extraRandomHeightIn, IBlockState woodMetadataIn,
			IBlockState leavesBlock, IBlockState fruitBlock) {
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		...
		if (!this.ensureGrowable(worldIn, rand, position, i)) {
			...
			this.setBlockAndNotifyAdequately(worldIn, position.up(i - 2).east().east().east().south(),
					this.woodMetadata);
			this.setBlockAndNotifyAdequately(worldIn, position.up(i - 2).south().south().south(), this.woodMetadata);

			for (int j = position.getY() + i - 2 - rand.nextInt(4); j > position.getY() + i / 3; j -= 3
					+ rand.nextInt(3)) {
				...
				if (j * j + k * k <= i) {
					...
					if (state.getBlock().isAir(state, worldIn, blockpos)
						...
						this.setBlockAndNotifyAdequately(worldIn, blockpos,
								worldIn.rand.nextInt(20) == 0 ? this.fruitBlock : this.leavesMetadata);
					}
				}
			}
		}
