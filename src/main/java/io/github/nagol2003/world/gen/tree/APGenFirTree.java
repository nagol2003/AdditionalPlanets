...
public class APGenFirTree extends APGenTreeBase {
	...
	public boolean generate (World worldIn, Random rand, BlockPos position) {
		...
		if (position.getY() >= 1 && position.getY() + treeHeight + 1 <= worldIn.getHeight()) {
			...
			if (!flag) {
				...
				if (state.getBlock()
						.canSustainPlant(state, worldIn, down, net.minecraft.util.EnumFacing.UP, (net.minecraft.block.BlockSapling) Blocks.SAPLING)
