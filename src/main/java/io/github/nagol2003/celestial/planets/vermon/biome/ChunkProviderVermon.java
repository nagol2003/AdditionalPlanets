...
import io.github.nagol2003.util.world.TestCaveGen;

public class ChunkProviderVermon extends ChunkProviderBase
{
    public static final IBlockState BLOCK_TOP_DEFAULT = InitBlocks.ROCK_1.getDefaultState();
    public static final IBlockState BLOCK_TOP_LIGHT = InitBlocks.ROCK_1.getDefaultState();
    public static final IBlockState BLOCK_TOP_DARK = InitBlocks.ROCK_1.getDefaultState();
    public static final IBlockState BLOCK_FILL = InitBlocks.ROCK_1.getDefaultState();
    public static final IBlockState BLOCK_LOWER = InitBlocks.ROCK_1.getDefaultState();
   ...
   // private final MapGenDungeonNeptune dungeonGenerator = new MapGenDungeonNeptune(new DungeonConfiguration(SolBlocks.NEPTUNE_DUNGEON_BRICK.getDefaultState(), 25, 8, 16, 5, 6, RoomBossNeptune.class, RoomTreasureNeptune.class));
    ...
    private final MapGenBaseMeta caveGenerator = new TestCaveGen();
    ...
    public ChunkProviderVermon(World par1World, long par2, boolean par4)
    {
    ...
    public void setBlocksInChunk(int chunkX, int chunkZ, ChunkPrimer primer)
    {
        ...
        for (int x = 0; x < ChunkProviderVermon.CHUNK_SIZE_X; x++)
        {
            for (int z = 0; z < ChunkProviderVermon.CHUNK_SIZE_Z; z++)
            {
                ...
                for (int y = 0; y < ChunkProviderVermon.CHUNK_SIZE_Y; y++)
                {
                    if (y < ChunkProviderVermon.MID_HEIGHT + yDev)
                    {
        ...
        for (int cx = chunkX - 2; cx <= chunkX + 2; cx++)
        {
            for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++)
            {
                for (int x = 0; x < ChunkProviderVermon.CHUNK_SIZE_X; x++)
                {
                    for (int z = 0; z < ChunkProviderVermon.CHUNK_SIZE_Z; z++)
                    {
                        if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * ChunkProviderVermon.CHUNK_SIZE_X + x, cz * ChunkProviderVermon.CHUNK_SIZE_Z + z) / ChunkProviderVermon.CRATER_PROB)
                        {
