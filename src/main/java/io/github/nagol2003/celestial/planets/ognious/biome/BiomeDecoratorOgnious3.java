...
import io.github.nagol2003.world.gen.tree.APGenTrunks;
...
public class BiomeDecoratorOgnious3 extends BiomeDecorator {
	...
	protected void genDecorations(Biome biomeIn, World world, Random random) {
		...
		for (int i = 0; i < this.treesPerChunk; i++) {
			if (random.nextInt(5) == 1) {
				...
				new APGenTrunks(Blocks.LOG, Blocks.LOG, Blocks.LOG).setCanGenerateOnSand().generate(world, random, blockpos);
			}
		}
		isDecorating = false;
	}
